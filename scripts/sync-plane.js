...
${(issue.acceptance_criteria || '').split('\n').filter(l => l.trim()).map(l => `- [ ] ${l.trim()}`).join('\n') || '- [ ] Pendiente de definir'}
...
async function push() {
  ...
  const files = fs.readdirSync(SPECS_DIR).filter(f => f.endsWith('.md') && f !== 'ISSUE_TEMPLATE.md')
...
async function createIssue(title, body) {
  try {
    const result = await api('work-items/', {
      method: 'POST',
      body: JSON.stringify({ name: title, description_html: body.replace(/\n/g, '<br>') }),
    })
